...
#-----------------------------------------------------------------------------# download
"""
    download(layers::Vector{Product}, aoi, every::Integer=5; kw...) --> file::String

Submits a job for the specified `layers` and `aoi`, then polls the job status every `every` seconds.
When the job completes successfully, downloads and returns the path to the output zipfile.
"""
function download(layers::Vector{Product}, aoi, every::Integer=5; kw...)
    ...
    while true
        ...
        if obj.status == "Succeeded"
            file = Downloads.download(obj.outputFile)
            return file
